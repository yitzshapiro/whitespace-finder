...
def filter_top_search_terms(combined_trend_data):
    ...
    filtered_terms = last_two_months[last_two_months['delta'] > 0].sum(axis=0).nlargest(5).index.str.replace('_trend', '').tolist()
...
async def main():
    ...
    for term in search_terms:
        try:
            ...
            if trend_data is None:
                ...
                continue
            combined_trend_data = update_combined_trend_data(trend_data, term, combined_trend_data)
    ...
    for term in filtered_terms:
        logging.info(f"Searching for '{term}' on Amazon using amazon-buddy-yitz-version...")
        result_file, result_count, list_length = search_on_amazon(term, number=40, country='US', filetype='csv')
        
        if result_file and result_count > 0:
            search_results.append({'term': term, 'result_file': result_file, 'result_count': result_count, 'listLength': list_length})
