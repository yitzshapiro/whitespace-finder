...
def search_on_amazon(search_term, number=100, country='US', filetype='csv'):
    logging.info(f"Searching for '{search_term}' on Amazon using amazon-buddy-yitz-version...")
    try:
        ...
        if match:
            ...
            if os.path.exists(filename):
                ...
                num_results = len(df)
                list_length = df.iloc[-1]['listLength'] if 'listLength' in df.columns else None
                logging.info(f"Found {num_results} results for '{search_term}' on Amazon.")
                return filename, num_results, list_length
            else:
                logging.warning(f"File '{filename}' does not exist, although it was stated in the output.")
                return None, 0, None
        else:
            logging.warning("Regex did not match any part of the output. Double-check the output format.")
